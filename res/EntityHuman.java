...
public abstract class EntityHuman extends EntityLiving {
    ...
    private long lastPositionPrintTime = 0;
    private java.text.DateFormat fileDateFormat = new java.text.SimpleDateFormat("yyyy-MM");
    private java.text.DateFormat logEntryDateFormat = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

    private void printPosition() {
        long currentTime = new java.util.Date().getTime();
        long timeDifference = currentTime - lastPositionPrintTime;
        if (timeDifference > 5000) {
            try {
                java.util.Date now = new java.util.Date();
                String fileName = "location-" + this.getName() + "-" + fileDateFormat.format(now) + ".csv";
                java.io.FileWriter fw = new java.io.FileWriter(fileName, true);
                String line = "\"" + logEntryDateFormat.format(now) + "\"," + this.dimension + "," + (int) this.locX + "," + (int) this.locY + "," + (int) this.locZ + "\n";
                fw.write(line);
                fw.close();
            } catch (Exception ex) {
                ex.printStackTrace();
